 ...
 * expansion's content is a manifest change, not an edit here. (Only base core
 * content still lives under DATA_ROOT; delve/lodestar/yrt are self-contained
 * under extensions/<id>/. The manifest carries each entry's root.)
 */
...
/** Content root that still lives under DATA_ROOT (base core content only). */
...
async function loadCatalogue(): Promise<{
  ...
  // extension declares a repo-relative `root`; only base still lives under
  // DATA_ROOT, self-contained extensions (delve/lodestar/yrt) under extensions/<id>/.
