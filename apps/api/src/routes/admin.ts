...
export const adminRoutes: FastifyPluginAsyncZod = async (server) => {
  ...
  server.patch(
    ...
    async (req, reply) => {
      ...
      await adminService
        .setUserRole(req.params.id, req.body.role, req.user!.id, req.ip)
  ...
  server.patch(
    ...
    async (req, reply) => {
      ...
      if (req.body.suspended) {
        await adminService
          .suspendUser(req.params.id, req.user!.id, req.ip)
          .catch(handleError(reply));
      } else {
        await adminService
          .unsuspendUser(req.params.id, req.user!.id, req.ip)
          .catch(handleError(reply));
      }
      if (reply.sent) return;
      return reply.status(204).send();
  ...
  server.get(
    ...
    async (req, reply) => {
      const result = await adminService
        .getAuditLog(100, req.query.search || undefined)
        .catch(handleError(reply));
      if (!result || reply.sent) return;
      return reply.status(200).send(result);
    },
  ...
  server.get(
    ...
    async (req, reply) => {
      const result = await adminService
        .getUserTimeseries(req.query.timeframe)
        .catch(handleError(reply));
      if (!result || reply.sent) return;
      return reply.status(200).send(result);
    },
  ...
  server.post(
    ...
    async (req, reply) => {
      const result = await maintenanceService
        .enableMaintenance(req.body.message, req.body.minutesUntilShutdown, req.user!.id, req.ip)
        .catch(handleError(reply));
      if (!result || reply.sent) return;
      return reply.status(200).send(result);
    },
  ...
  server.post(
    ...
    async (req, reply) => {
      const result = await registrationLockService
        .lockRegistration(req.body.message, req.user!.id, req.ip)
        .catch(handleError(reply));
      if (!result || reply.sent) return;
      return reply.status(200).send(result);
    },
  ...
  server.delete('/registration-lock', async (req, reply) => {
    await registrationLockService
      .unlockRegistration(req.user!.id, req.ip)
      .catch(handleError(reply));
    if (reply.sent) return;
