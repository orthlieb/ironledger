...
describe('reconcileGlobalValues', () => {
	const manaOnly = new Map([['mana', { maxValue: 10 }]]);
	...
	it('passes values through unchanged when the catalogue is empty', () => {
		...
		expect(reconcileGlobalValues({ mana: '5', oldKey: '7' }, new Map())).toEqual({
			mana: '5',
			oldKey: '7',
		});
	});
	...
	it('drops counter ids the catalogue does not know', () => {
		expect(reconcileGlobalValues({ mana: '5', extinct: '99' }, manaOnly)).toEqual({ mana: '5' });
	});

	it('clamps values above the canonical max', () => {
		expect(reconcileGlobalValues({ mana: '50' }, manaOnly)).toEqual({ mana: '10' });
	});

	it('clamps negative values to 0', () => {
		expect(reconcileGlobalValues({ mana: '-3' }, manaOnly)).toEqual({ mana: '0' });
	});

	it('floors fractional values', () => {
		expect(reconcileGlobalValues({ mana: '4.7' }, manaOnly)).toEqual({ mana: '4' });
	});

	it('drops non-numeric strings', () => {
		expect(reconcileGlobalValues({ mana: 'wizard' }, manaOnly)).toEqual({});
	});
	...
	it('does not clamp array-typed maxValue (depends on per-asset abilities)', () => {
		...
		expect(reconcileGlobalValues({ ticks: '12' }, arrayMax)).toEqual({ ticks: '12' });
	});

	it('handles a missing maxValue (counter with no cap)', () => {
		const uncapped = new Map([['mana', {}]]);
		expect(reconcileGlobalValues({ mana: '99' }, uncapped)).toEqual({ mana: '99' });
	});
});
...
describe('buildGlobalCounterRegistry', () => {
	...
	it('first declaration wins when assets agree on canonical properties', () => {
		...
		const reg = buildGlobalCounterRegistry([asset('a/1', [f1]), asset('a/2', [f2])], onMismatch);
