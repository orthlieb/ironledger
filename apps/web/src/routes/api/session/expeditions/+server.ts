/**
 * SvelteKit BFF proxy for /api/session/expeditions
 *
 * PATCH → forward to Fastify PATCH /api/v1/session/expeditions
 */
...
export const PATCH: RequestHandler = async ({ locals, request }) => {
	...
	const res = await fetch(`${INTERNAL_API_URL}/api/v1/session/expeditions`, {
		method: 'PATCH',
		headers: {
			...authHeader(locals),
