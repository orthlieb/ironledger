/**
 * SvelteKit BFF proxy for /api/session/communities
 *
 * PATCH → forward to Fastify PATCH /api/v1/session/communities
 */
...
export const PATCH: RequestHandler = async ({ locals, request }) => {
	...
	const res = await fetch(`${INTERNAL_API_URL}/api/v1/session/communities`, {
		method: 'PATCH',
		headers: {
			...authHeader(locals),
