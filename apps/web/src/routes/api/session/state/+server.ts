/**
 * SvelteKit BFF proxy for /api/session/state
 *
 * PATCH → forward to Fastify PATCH /api/v1/session/state
 */
...
export const PATCH: RequestHandler = async ({ locals, request }) => {
	...
	const res = await fetch(`${INTERNAL_API_URL}/api/v1/session/state`, {
		method: 'PATCH',
		headers: {
			...authHeader(locals),
