<script lang="ts">
	...
	$effect(() => {
		if (editingNotesFieldId && notesTextareaEl) notesTextareaEl.focus();
	});
	...
	const visibleRarities = $derived(
		findRaritiesForAsset(asset.assetId).filter(
			(r) => isSourceEnabled(r.source) || asset.rarityId === r.id,
		),
	);
	...
	const selectableLabel = $derived((definition.selectableLabel as string | undefined) ?? 'Known');
	...
	const assetDescription = $derived((definition.description as string | null | undefined) ?? null);
	...
	const counterField = $derived((definition.customFields ?? []).find((f) => f.type === 'counter'));
	...
	function effectiveCf(
		cf: import('$lib/types.js').CustomFieldDef,
	): import('$lib/types.js').CustomFieldDef {
				...
				<div class="asset-notes-row">
					...
					{:else}
						...
						<div
							...
							onkeydown={(e) => {
								if (e.key === 'Enter') editingNotesFieldId = field.id;
							}}
						>
							{#if value.trim()}
...
<style>
	...
	.asset-name-icon :global(svg) {
		width: 100%;
		height: 100%;
		fill: currentColor;
	}
	.asset-name-icon :global(svg path) {
		fill: currentColor;
	}
	...
	.asset-description :global(strong) {
		color: var(--text);
	}
	.asset-description :global(br) {
		display: block;
		margin-bottom: 0.4em;
		content: '';
	}
	...
	.ability-text :global(p:last-child) {
		margin-bottom: 0;
	}
	...
	.factors-example-table th:nth-child(2) {
		width: 32px;
		text-align: center;
	}
	.factors-example-table th:nth-child(1) {
		width: 28%;
	}
	...
	.asset-notes-textarea:focus {
		border-color: var(--focus-ring, #e8a13b);
	}
	.asset-notes-display {
		font-family: var(--font-ui);
		font-size: 0.78rem;
	...
	.asset-notes-display :global(p) {
		margin: 0 0 2px;
	}
	.asset-notes-display :global(p:last-child) {
		margin-bottom: 0;
	}
	...
	.asset-notes-display :global(ol) {
		margin: 1px 0;
		padding-left: 1.2em;
	}
	.asset-notes-display :global(li) {
		margin-bottom: 1px;
	}
	.asset-notes-display :global(strong) {
		font-weight: 700;
		color: var(--text);
	}
	.asset-notes-display :global(em) {
		font-style: italic;
	}
	.asset-notes-display :global(br) {
		display: block;
		margin-bottom: 2px;
		content: '';
	}
