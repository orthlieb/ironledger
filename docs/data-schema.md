...
For Yrt homebrew extensions (mana, Touched assets, cantrips, Yrt-specific oracles and foes), see **[yrt/data-schema-yrt.md](yrt/data-schema-yrt.md)**.
...
| `data-resource` | One of: `momentum`, `health`, `spirit`, `supply`, `bonds`, `failures`, `xp` (plus `mana` in [Yrt](yrt/data-schema-yrt.md#mana-resource)) |
...
See [Yrt extensions](yrt/data-schema-yrt.md#mana-resource) for the `mana` resource (0–10, amber `#f59e0b`).
...
| `category`          | string           | yes      | One of: `"Combat Talent"`, `"Companion"`, `"Path"`, `"Ritual"` (plus `"Touched"` in [Yrt](yrt/data-schema-yrt.md#touched-assets))                                                                                                                                                                                                      |
...
Yrt adds Touched assets and ritual cantrips — see [yrt/data-schema-yrt.md](yrt/data-schema-yrt.md#touched-assets).
...
Yrt adds additional oracle tables with structured values — see [yrt/data-schema-yrt.md](yrt/data-schema-yrt.md#yrt-oracles).
...
| `escalates`        | boolean          | no       | If `true`, this foe's harm escalates during combat rather than being fixed by rank. Surfaces a +/− harm counter on the FoeCard. The current level is stored as `currentHarm` on the encounter record. See [Yrt extensions](yrt/data-schema-yrt.md#escalating-harm-yrt-extension).                                                                    |
| `escalatesDefense` | boolean          | no       | If `true`, this foe has a mana shield that absorbs strikes. Defense starts at the rank's progressPerHit cap and erodes toward 1 on each Miss. Surfaces a +/− defense counter on the FoeCard. The current level is stored as `currentDefense` on the encounter record. See [Yrt extensions](yrt/data-schema-yrt.md#escalating-defense-yrt-extension). |
...
| `currentHarm`    | number  | _(Optional)_ Current escalating harm level. Only meaningful when `FoeDef.escalates` is `true`. Defaults to `1` when absent. See [Yrt extensions](yrt/data-schema-yrt.md#escalating-harm-yrt-extension).                                                           |
| `currentDefense` | number  | _(Optional)_ Current escalating defense level. Only meaningful when `FoeDef.escalatesDefense` is `true`. Absent = full cap (`FOE_RANKS[effectiveRank].progressPerHit`). Minimum 1. See [Yrt extensions](yrt/data-schema-yrt.md#escalating-defense-yrt-extension). |
