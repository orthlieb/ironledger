...
| `preamble` | string or null | no | Prerequisite or flavour text (e.g., `"If you wield a bow."`) — displayed on the asset card before the ability list |
| `postamble` | string or null | no | Explanatory text displayed on the asset card *after* the ability list (e.g., asset-specific constraints or notes) |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `enabled` | boolean | yes | `true` if the ability starts active (first ability is usually `true`) |
