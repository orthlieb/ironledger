...
For Yrt homebrew extensions (mana, Touched assets, cantrips, Yrt-specific oracles and foes), see **[YRT/DATA_FORMAT_YRT.md](YRT/DATA_FORMAT_YRT.md)**.
...
| `data-resource` | One of: `momentum`, `health`, `spirit`, `supply`, `bonds`, `failures`, `xp` (plus `mana` in [Yrt](YRT/DATA_FORMAT_YRT.md#mana-resource)) |
...
See [Yrt extensions](YRT/DATA_FORMAT_YRT.md#mana-resource) for the `mana` resource (0–10, amber `#f59e0b`).
...
| `category` | string | yes | One of: `"Combat Talent"`, `"Companion"`, `"Path"`, `"Ritual"` (plus `"Touched"` in [Yrt](YRT/DATA_FORMAT_YRT.md#touched-assets)) |
...
Yrt adds Touched assets and ritual cantrips — see [YRT/DATA_FORMAT_YRT.md](YRT/DATA_FORMAT_YRT.md#touched-assets).
...
Yrt adds additional oracle tables with structured values — see [YRT/DATA_FORMAT_YRT.md](YRT/DATA_FORMAT_YRT.md#yrt-oracles).
